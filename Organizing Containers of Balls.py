...
def organizingContainers(container):
    ...
    total_balls_of_type = [sum(row) for row in container]
    total_balls_in_container = [sum(col) for col in zip(*container)]

    # Sort the lists
    total_balls_of_type.sort()
    total_balls_in_container.sort()

    # Check if the two sorted lists are equal
    if total_balls_of_type == total_balls_in_container:
        return "Possible"
    else:
        return "Impossible"
